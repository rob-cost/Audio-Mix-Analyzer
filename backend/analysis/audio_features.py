...
def get_dsp_features(audio_bytes, sr=None):

    """
    Get DSP features paramters for given audio bytes.
    
    Arguments: audio_bytes : bytes of audio file, sr : sample rate (None for original)

    Return: List of DSP features
    ...
    # Create mono signal for integrated loudness (industry standard)
    mono = (left + right) / 2


    # Temporal features
    zero_crossing_rate = librosa.feature.zero_crossing_rate(mono).mean()


    # Stereo / spatial features
    stereo_corr = np.corrcoef(left, right)[0,1]
    mid = (left + right) / 2
    side = (left - right) / 2
    ...
    dsp_features.append({
        ...
        "stereo_correlation": float(stereo_corr),
        "mid_level": float(mid_level),
        "side_level": float(side_level)
    })
    return dsp_features[0]